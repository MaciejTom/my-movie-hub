...
function ActorSlider({ images }) {
  ...
  return (
    ...
    <Wrapper>
    <div onClick={() => getImg("")}className={imgState.modal ? `modal open` : `modal`}>
        <img src={`${IMAGE_BASE_URL}${BACKDROP_SIZE}${imgState.imageSrc}`}/>
        
      </div>
      <div className="title">
        <h1>Image{images.length > 1 ? "s" : null}</h1>
      </div>
      <Content>
        <div></div>
        {images.map((img, imgIndex) => {
          ...
          return (
            <div key={img.id} className={`${position} imgDiv`}>
              <Image src={`${IMAGE_BASE_URL}${POSTER_SIZE}${img.file_path}`} onClick={() => getImg(img)} />
              {img.media.title && (
...
// https://api.themoviedb.org/3/person/17838/tagged_images?api_key=b7b239fc57b95047aaeec74f47f5ee93&language=en-US&page=1
// http://image.tmdb.org/t/p/w780//lNXkxjiVwWKXalBcDCpntXBBfOh.jpg
// /tcNniniS4rfqrLH0oORikJfnIwY.jpg
